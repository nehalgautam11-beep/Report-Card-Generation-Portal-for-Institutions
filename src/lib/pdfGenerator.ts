import PDFDocument from "pdfkit";
import fs from "fs";
import path from "path";

export interface StudentData {
  name: string;
  fatherName: string;
  motherName: string;
  className: string;
  dob: string;
  qualities: string;
  workingDays: number;
  attendedDays: number;
  subjects: { name: string; marks: SubjectMarks }[];
  remarks?: string;
}

export interface SubjectMarks {
  periodicRaw: number; // Out of 20
  enrichment: number;  // Out of 10
  term2: number;       // Out of 80
}

const getGrade = (percentage: number): string => {
  if (percentage >= 91) return "A1";
  if (percentage >= 81) return "A2";
  if (percentage >= 71) return "B1";
  if (percentage >= 61) return "B2";
  if (percentage >= 51) return "C1";
  if (percentage >= 41) return "C2";
  if (percentage >= 33) return "D";
  return "E";
};

const toTitleCase = (str: string): string => {
  return (str || "")
    .toLowerCase()
    .split(" ")
    .map((word) => word.charAt(0).toUpperCase() + word.slice(1))
    .join(" ");
};

export const generateReportCardPDF = (data: StudentData, logoBuffer?: Buffer): Promise<Buffer> => {
  return new Promise((resolve, reject) => {
    try {
      const doc = new PDFDocument({ size: "A4", margin: 0 });
      const buffers: Buffer[] = [];
      doc.on("data", buffers.push.bind(buffers));
      doc.on("end", () => resolve(Buffer.concat(buffers)));

      const BLUE_BRAND = "#1f497d";
      const BLUE_HEADER = "#e3f2fd";
      const GREEN_ATT = "#2e7d32";
      const BLACK = "#000000";
      const WHITE = "#ffffff";
      const MARGIN_X = 45;
      const PAGE_W = doc.page.width;

      // ─── HEADER ────────────────────────────────────────────────────
      doc.rect(0, 0, PAGE_W, 85).fill(BLUE_HEADER);

      if (logoBuffer) {
        try { doc.image(logoBuffer, MARGIN_X, 12, { width: 68 }); } catch { }
      } else {
        const logoPath = path.join(process.cwd(), "public", "gis_logo.png");
        if (fs.existsSync(logoPath)) {
          try { doc.image(logoPath, MARGIN_X, 12, { width: 68 }); } catch { }
        }
      }

      doc.font("Helvetica-Bold").fontSize(26).fillColor(BLUE_BRAND);
      doc.text("GLOBAL INNOVATIVE SCHOOL", 140, 32);

      // ─── REPORT CARD TITLE ─────────────────────────────────────────
      doc.font("Helvetica-Bold").fontSize(18).fillColor(BLUE_BRAND);
      doc.text("REPORT CARD", 0, 105, { align: "center", width: PAGE_W });

      // ─── STUDENT DETAILS ───────────────────────────────────────────
      let y = 135;
      const colL = MARGIN_X;
      const colR = 305;
      const lineW = 185;

      const drawDetail = (label: string, value: string, x: number, ly: number, colonX: number) => {
        doc.font("Helvetica-Bold").fontSize(11).fillColor(BLUE_BRAND).text(label, x, ly);
        doc.text(":", x + colonX, ly);
        doc.font("Helvetica").fontSize(13).fillColor(BLACK).text(toTitleCase(value), x + colonX + 12, ly - 1);
        doc.moveTo(x + colonX + 10, ly + 14).lineTo(x + colonX + lineW, ly + 14)
          .strokeColor("#cccccc").lineWidth(0.8).stroke();
      };

      drawDetail("NAME", data.name, colL, y, 65);
      drawDetail("Mother's Name:", data.motherName, colR, y, 95);
      y += 26;
      drawDetail("CLASS", data.className, colL, y, 65);
      drawDetail("Father's Name:", data.fatherName, colR, y, 95);
      y += 26;
      drawDetail("DOB", data.dob, colL, y, 65);

      y += 38;

      // ─── MARKS TABLE ───────────────────────────────────────────────
      const tableX = MARGIN_X;
      const tableW = PAGE_W - MARGIN_X * 2;
      const colPos = [0, 110, 215, 325, 425].map(v => v + tableX);

      // Header row — blue background, white bold text
      const hH = 40;
      doc.rect(tableX, y, tableW, hH).fill(BLUE_BRAND);

      // Vertical dividers in header
      doc.strokeColor(WHITE).lineWidth(0.5);
      for (let i = 1; i < colPos.length; i++) {
        doc.moveTo(colPos[i], y).lineTo(colPos[i], y + hH).stroke();
      }
      // Outer border of header
      doc.rect(tableX, y, tableW, hH).strokeColor(BLUE_BRAND).lineWidth(1).stroke();

      doc.font("Helvetica-Bold").fontSize(9).fillColor(WHITE);
      doc.text("Subject", tableX + 5, y + 15);
      doc.text("Periodic Test - 2\n(Out of 10)", colPos[1] + 5, y + 8, { width: 95 });
      doc.text("Subject Enrichment\n(Out of 10)", colPos[2] + 5, y + 8, { width: 95 });
      doc.text("Term - 2 Examination\n(Out of 80)", colPos[3] + 5, y + 8, { width: 95 });
      doc.text("Overall Marks\n(Out of 100)", colPos[4] + 5, y + 8, { width: 75 });

      y += hH;
      let totalAccum = 0;

      data.subjects.forEach(st => {
        const pRaw = Number(st.marks.periodicRaw) || 0;
        const p = Math.round(pRaw / 2); // 20 -> 10
        const e = Number(st.marks.enrichment) || 0;
        const t = Number(st.marks.term2) || 0;
        const sum = p + e + t;
        totalAccum += sum;

        doc.rect(tableX, y, tableW, 28).fill(WHITE).strokeColor(BLUE_BRAND).lineWidth(1).stroke();
        for (let i = 1; i < colPos.length; i++) {
          doc.moveTo(colPos[i], y).lineTo(colPos[i], y + 28).strokeColor(BLUE_BRAND).stroke();
        }

        doc.font("Helvetica-Bold").fontSize(11).fillColor(BLACK).text(st.name, tableX + 8, y + 8);
        doc.font("Helvetica").fontSize(12).fillColor(BLACK).text(p.toString(), colPos[1] + 8, y + 8);
        doc.text(e.toString(), colPos[2] + 8, y + 8);
        doc.text(t.toString(), colPos[3] + 8, y + 8);
        doc.text(sum.toString(), colPos[4] + 8, y + 8);
        y += 28;
      });

      y += 24;

      // ─── STATS & ATTENDANCE ────────────────────────────────────────
      const tPoss = data.subjects.length * 100;
      const perc = tPoss > 0 ? (totalAccum / tPoss) * 100 : 0;
      const grad = getGrade(perc);

      const drawS = (label: string, value: string, sy: number) => {
        doc.font("Helvetica-Bold").fontSize(12).fillColor(BLUE_BRAND).text(label, MARGIN_X, sy);
        doc.font("Helvetica").fontSize(12).fillColor(BLACK).text(value, MARGIN_X + 135, sy + 1);
        doc.moveTo(MARGIN_X + 130, sy + 14).lineTo(MARGIN_X + 270, sy + 14).strokeColor(BLUE_BRAND).lineWidth(1).stroke();
      };

      drawS("Overall Marks:", `${totalAccum} / ${tPoss}`, y);
      drawS("Overall Percentage:", `${perc.toFixed(2)}%`, y + 28);
      drawS("Overall Grade:", grad, y + 56);

      // Attendance table (right side) — with 3 rows including Attendance Percentage
      const aX = 330;
      const aW = PAGE_W - MARGIN_X - aX;
      const attY = y;

      doc.font("Helvetica-Bold").fontSize(12).fillColor(BLUE_BRAND)
        .text("Attendance", aX, attY, { width: aW, align: "center" });

      const attTableY = attY + 18;
      const attRowH = 25;

      const attendancePerc = data.workingDays > 0
        ? ((data.attendedDays / data.workingDays) * 100).toFixed(2) + "%"
        : "0.00%";

      const attRows: [string, string, boolean][] = [
        ["Total Working Days", data.workingDays.toString(), false],
        ["Number of Days Attended", data.attendedDays.toString(), false],
        ["Attendance Percentage", attendancePerc, true],
      ];

      attRows.forEach(([label, val, bold], idx) => {
        const ry = attTableY + idx * attRowH;
        doc.rect(aX, ry, aW, attRowH).fill(WHITE).strokeColor(BLUE_BRAND).lineWidth(1).stroke();
        doc.moveTo(aX + 140, ry).lineTo(aX + 140, ry + attRowH).strokeColor(BLUE_BRAND).stroke();

        const labelFont = bold ? "Helvetica-Bold" : "Helvetica-Bold";
        const labelColor = GREEN_ATT;
        doc.font(labelFont).fontSize(9).fillColor(labelColor).text(label, aX + 8, ry + 8);

        const valFont = bold ? "Helvetica-Bold" : "Helvetica";
        const valColor = bold ? GREEN_ATT : BLACK;
        doc.font(valFont).fontSize(11).fillColor(valColor).text(val, aX + 148, ry + 7);
      });

      y += 95;

      // ─── FOOTER ────────────────────────────────────────────────────
      doc.rect(0, y, PAGE_W, 400).fill(BLUE_HEADER);

      const bH = 95;
      doc.rect(MARGIN_X, y + 15, PAGE_W - MARGIN_X * 2, bH).fill(WHITE).strokeColor(BLUE_HEADER).stroke();
      doc.font("Helvetica-Bold").fontSize(11).fillColor(BLUE_BRAND).text("Teacher's Remark:", MARGIN_X + 12, y + 25);
      if (data.remarks) {
        doc.font("Helvetica").fontSize(11).fillColor(BLACK)
          .text(data.remarks, MARGIN_X + 12, y + 42, { width: 480, lineGap: 5 });
      }

      y += bH + 65;

      // Signature lines
      const sW = 160;
      doc.strokeColor(BLACK).lineWidth(1.2);
      doc.moveTo(MARGIN_X, y).lineTo(MARGIN_X + sW, y).stroke();
      doc.moveTo(PAGE_W - MARGIN_X - sW, y).lineTo(PAGE_W - MARGIN_X, y).stroke();

      doc.font("Helvetica").fontSize(12).fillColor(BLACK);
      doc.text("Class Teacher", MARGIN_X, y + 8, { width: sW, align: "center" });
      doc.text("School Stamp", 0, y + 8, { width: PAGE_W, align: "center" });
      doc.text("Principal", PAGE_W - MARGIN_X - sW, y + 8, { width: sW, align: "center" });

      doc.end();
    } catch (e) {
      reject(e);
    }
  });
};

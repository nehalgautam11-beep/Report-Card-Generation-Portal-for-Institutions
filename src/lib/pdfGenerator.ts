...
export const generateReportCardPDF = (data: StudentData, logoBuffer?: Buffer): Promise<Buffer> => {
  return new Promise((resolve, reject) => {
    try {
      ...
      // Header: light blue bg, bold blue text, blue borders
      const hH = 40;
      doc.rect(tableX, y, tableW, hH).fill(BLUE_HEADER);
      doc.rect(tableX, y, tableW, hH).strokeColor(BLUE_BRAND).lineWidth(1).stroke();
      doc.strokeColor(BLUE_BRAND).lineWidth(0.8);
      ...
      doc.font("Helvetica-Bold").fontSize(12).fillColor(BLUE_BRAND)
        .text("Attendance", aX, attBaseY, { width: aW, align: "center" });

      const attTableY = attBaseY + 18;
      const attRowH = 25;
      const attDivX = 140;

      const attPerc = data.workingDays > 0
      ...
      const attRows: [string, string, boolean][] = [
        ...
        ["Attendance Percentage", attPerc, true],
      ];

      attRows.forEach(([label, val, isBold], idx) => {
        const ry = attTableY + idx * attRowH;
        doc.rect(aX, ry, aW, attRowH).fill(WHITE).strokeColor(BLUE_BRAND).lineWidth(1).stroke();
        doc.moveTo(aX + attDivX, ry).lineTo(aX + attDivX, ry + attRowH).strokeColor(BLUE_BRAND).lineWidth(0.8).stroke();
        ...
        doc.font(isBold ? "Helvetica-Bold" : "Helvetica").fontSize(11)
          .fillColor(isBold ? GREEN_ATT : BLACK).text(val, aX + attDivX + 8, ry + 7);
      });

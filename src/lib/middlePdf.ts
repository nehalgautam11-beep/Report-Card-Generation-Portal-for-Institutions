...
const getPromotedClass = (currentClass: string): string => {
  ...
  if (cls === 'Nursery' || cls === 'nursery') return 'KG 1';
...
const getLayoutConfig = (reportLevel?: ReportLevel) => {
  if (reportLevel === "middle") {
    return {
      headerHeight: 110,
      titleY: 120,
      detailsStartY: 155,
...
export const generateReportCardPDF = (
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      const MARGIN_X = 45;
      const layout = getLayoutConfig(reportLevel);

      // Choose spacing based on the selected school level.
      doc.rect(0, 0, PAGE_W, layout.headerHeight).fill(BG_BLUE);
      doc.rect(0, layout.headerHeight, PAGE_W, PAGE_H - layout.headerHeight).fill(WHITE);
      ...
      doc.text("REPORT CARD", 0, layout.titleY, { align: "center", width: PAGE_W });

      // 4. STUDENT DETAILS (Labels font increased to 12)
      let curY = layout.detailsStartY;
      ...
      const remarkBoxHeight = layout.remarkBoxHeight;
      curY += 15;
      ...
      curY += remarkBoxHeight + layout.signatureGap + 10;
